...
function Compiler:codeCall(ast)
  ...
  if (hasDefaultParam and #args ~= #func.params - 1) and #args ~= #func.params then
...
function Compiler:codeExp(ast)
  ...
  elseif ast.tag == "logicalop" then
    local opcode = ast.op == "and" and "jumpZP" or "jumpNZP"
    self:codeExp(ast.e1)
  ...
  elseif ast.tag == "local" then
    ...
    for i = 1, #self.params do
      if self.params[i] == ast.name then
        error("Local variable '" .. ast.name .. "' already defined as parameter")
      end
    end
    if ast.init then
...
function Compiler:codeFunction(ast)
  ...
  if func and #func.code > 0 then
    print("WARN: Function " .. ast.name .. " is being redefined")
  end
